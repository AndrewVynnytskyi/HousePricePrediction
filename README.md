...
this repo's own `src/` pipeline — every accuracy number below reproduced
to the digit against the original exploratory notebook's cached output
...
- **PCA doesn't help.** Every model's RMSE gets worse (or stays flat) under
  ...
  The original exploratory notebook saw a much larger gap on its hardware
  (~56 min vs ~9 min, roughly 6x); the accuracy numbers reproduce exactly
  run to run, but grid-search wall-clock time doesn't — it's a function of
  the machine it runs on, not of the algorithm.
